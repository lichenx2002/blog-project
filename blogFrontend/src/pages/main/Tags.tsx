...
const Tags: React.FC = () => {
    ...
    const generateAnimationParams = useCallback(() => {
        ...
        // 随机生成Y轴位置（在屏幕高度的20%-80%范围内）
        const startY = windowHeight * 0.2 + Math.random() * windowHeight * 0.6;

        // 设置终点X坐标（在屏幕宽度的30%-70%范围内）
        const endX = windowWidth * 0.3 + Math.random() * windowWidth * 0.4;
        ...
        return {
            ...
            delay: `${Math.random() * 3000}ms` // 0-3秒的随机延迟
        };
    }, []);
